import 'package:flutter/widgets.dart';

class ColorsManager {
  // static const Color backgroundColor = Color(0xffF5F5F5);
  // static const Color backgroundColor = Color(0xffEFF3FB);
  static const Color backgroundColor = Color(0xffF5F5F5);
  // static const Color backgroundColor = Color(0xfff4f5f9);
  static const Color backgroundColorDark = Color(0xff1A1A1A);
  static const Color kBackgroundColor = Color(0xFFF1EFF1);
  // static const Color kPrimaryColor = Color(0xFF213555);
  static const Color kPrimaryColor = Color(0xFF3A59D1);
  // static const Color kPrimaryColor = Color(0xFF211C84);
  // static const Color kPrimaryColor = Color(0xFF3952FF);
  // 0xFF4335A7
  // 0xff1F509A
  static const Color yellow = Color.fromARGB(255, 44, 58, 69);
  static const Color textFormField = Color(0xFFF5F5F5);
  static const Color textFormFieldDark = Color(0xFF252525);
  static const Color red = Color(0xFFC71919);
  static const Color containerColor = Color(0xFFF5F5F5);
  static const Color containerColorDark = Color(0xFFF5F5F5);
  static const Color appBarColor = Color(0xffF5F5F5);
  static const Color appBarColorDarek = Color(0xFF252525);
  static const Color white = Color(0xffF5F5F5);
  static const Color grayIcon = Color(0xff444444);
  static const Color gray = Color(0xFFA2A2A2);
  static const Color grayBorder = Color(0xFFD9D9D9);
  static const Color dark = Color(0xFF303030);
  // static const Color textFormField = Color(0xffF5F5F5);
  static const Color darkBlue = Color(0xFF242424);
  static const Color lightGray = Color(0xFFC2C2C2);
  static const Color lighterGray = Color(0xFFEDEDED);
}

// F5F5F5const Color.fromARGB(255, 26, 26, 26)
